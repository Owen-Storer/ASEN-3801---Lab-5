...
u0_1 = [0;0;0;0]; % control surfaces are all zero 
x0_1(7) = V(1); % inertial velocity in body x is set to airspeed 

% call ode45 to simulate the equations of motion 
[t1,aircraftStateArray] = ode45(@(t, var) AircraftEOM(t, var, u0_1, Vwind_inertial, aircraft_parameters), [0,200], x0_1);

control_input_array = zeros(length(t1), 4);
...
% call ode45 to simulate the equations of motion 
[t2,aircraftStateArray2] = ode45(@(t, var) AircraftEOM(t, var, u0_2, Vwind_inertial, aircraft_parameters), [0,200], x0_2);

control_input_array2 = repmat(u0_2.', length(t2),1);
...
%% Case 3
x0_3 = [     0;      0; -1800;     % m
             deg2rad(15);   deg2rad(-12);   deg2rad(270);     % deg
             19;     3;    -2;     % m/s
            deg2rad(.08);  deg2rad(-0.2);     0   ]; % deg/s
u0_3 = [deg2rad(5); deg2rad(2); deg2rad(-13); .3];            % 3 deg 1 rad

% call ode45 to simulate the equations of motion 
[t3,aircraftStateArray3] = ode45(@(t, var) AircraftEOM(t, var, u0_3, Vwind_inertial, aircraft_parameters), [0,200], x0_3);

control_input_array3 = repmat(u0_3.', length(t3),1 );
